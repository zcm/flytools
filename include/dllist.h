/** @file dllist.h
 * This is the header file for the doubly linked list abstract data type provided by the
 * Flytools. To use this type, simply include this header file in your source code.
 *
...
#include <stdlib.h>
// enclosing this in an ifdef fixes a compiler error with old compilers about
// relative includes (i.e., Turbo C)
...
struct DOUBLE_LINKED_LIST {
	struct DOUBLE_LINKED_LIST_NODE *head; //!< The header or sentinel node for this linked list.
	int size; //!< The number of nodes contained within this linked list.
	void *(*alloc_callback)(size_t); //!< The stored allocation routine for allocating new nodes.
	void (*free_callback)(void *); //!< The stored freeing routine for destroying the structure and nodes. If null, the standard free() will be used.
};

// type aliases
typedef struct DOUBLE_LINKED_LIST double_linked_list;
typedef double_linked_list dllist;

// allocation
/**
 * Allocates a doubly linked list with the specified allocation routine.
 * Does no initialization.
 * @param alloc_callback the function pointer to the allocation routine for allocating the new list
...
dllist *dllist_alloc();
/**
 * Frees all of the memory associated with this doubly linked list using
 * the specified freeing routine. Any nodes and elements left in the array
 * will be freed with the same freeing routine.
 * @param free_callback the function pointer to the freeing routine for freeing the specified list
 */
void dllist_destroy_with(dllist *list, void (*free_callback)(void *));
/**
 * Frees all of the memory associated with this doubly linked list. Any
 * nodes and elements left in the array are attempted to be freed
 * with a call to free().
...
// initialization
/**
 * Initializes a previously allocated doubly list that was allocated with
 * the specified allocation callback. This callback is used again to allocate
 * the sentinel node in the list. Additionally, the callback specified here is
 * stored in the list for further use, i.e. when allocating new nodes.
 * @param list the doubly linked list to initialize
 * @param alloc_callback the allocation callback that was used to allocate this list structure
 */
void dllist_init_with(dllist *list, void *(*alloc_callback)(size_t));
/**
 * Initializes a previously allocated doubly linked list. This will
 * allocate the proper header node and set up the links.
 * @param list the doubly linked list to initialize
 */
 ...
 * @param alloc_callback the allocation routine for which to allocate this doubly linked list
...
/**
 * Sets the freeing routine for this linked list. Nodes that are freed will
 * be freed with a call to the specified function. This should be set at the
 * time that the linked list is allocated, or at least before any nodes are
...
/**
 * Inserts the specified node after the header node (which is considered
...
/**
 * Inserts the specified node before the header node (which is considered
...
/**
 * Removes the given node from the list and returns its data. This function
 * has undefined results if the node is not actually contained in the given list.
...
/**
 * Allocates a node to insert into a list at the left and copies the element pointer
 * into the node inserted. This will effectively insert the element into the list.
 ...
 * @param elem a pointer to the element that will be inserted into the list
 */
void dllist_insert_left(dllist *list, void *elem); // alias function
/**
 * Alias function for dllist_insert_elem_after_head().
 * @param list the list in which to insert the element
 * @param elem a pointer to the element that will be inserted into the list
 */
void dllist_push(dllist *list, void *elem); // alias function
/**
 * Allocates a node to insert into a list at the right and copies the element pointer
 * into the node inserted. This will effectively insert the element into the list.
 * @param list the list in which to insert the element
 * @param elem a pointer to the element that will be inserted into the list
 */
 ...
 * @param elem a pointer to the element that will be inserted into the list
 */
void dllist_insert_right(dllist *list, void *elem); // alias function
/**
 * Alias function for dllist_insert_elem_before_head().
 * @param list the list in which to insert the element
 * @param elem a pointer to the element that will be inserted into the list
 */
void dllist_enqueue(dllist *list, void *elem); // alias function
/**
 * Removes the first node from the list and returns the element pointer
 * stored in the node. The node will be freed.
...
/**
 * Removes the last node from the list and returns the element pointer
 * stored in the node. The node will be freed.
 * @param list the list from which to remove the element
 * @return a generic pointer to the element removed from the list
 */
...
// other functions
/**
 * Removes all nodes from the specified list, doing nothing to the
 * respective nodes' element pointers.
 ...
 * @param list the list to empty
 * @param callback the callback function to call for every element pointer in the list
 */
